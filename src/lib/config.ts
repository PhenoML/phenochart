...
export async function getConfig(): Promise<PhenoConfig | null> {
  ...
  const raw = result[CONFIG_KEY] as Record<string, unknown> | undefined;

  // Migrate old username/password configs: clear them so user re-configures
  if (raw && ('username' in raw || 'password' in raw)) {
    await browser.storage.local.remove(CONFIG_KEY);
    return null;
  }

  const config = raw as Partial<PhenoConfig> | undefined;
  if (
    !config?.instanceUrl ||
    !config?.clientId ||
    !config?.clientSecret ||
...
export function createClient(config: PhenoConfig) {
